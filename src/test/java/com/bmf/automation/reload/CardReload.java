		...
		try {
			...
			if (page == null) {
				if (orderType.contains("reload") || orderType.contains("unload")) {
					WebElement urSwitch = getDriver().findElement(ElementReload.SWITCH_UNLOAD_RELOAD.get());
					urSwitch.click();
					Thread.sleep(5000);
				}
			}
			...
			for (int i = 0; i <= 3; i++) {
				...
				if (i == 3) {
					WebElement kitInput = null;
					WebElement cardNumber = null;
					if (bmfurl.contains("forex-card")) {
						if (card.toLowerCase().contains("bookmyforex")) {
							kitInput = getDriver().findElement(ElementReload.CMS_KIT_CARD.get());
							kitInput.clear();
							kitInput.sendKeys(kit);
						} else {
							cardNumber = getDriver().findElement(ElementReload.CMS_KIT_CARD.get());
							cardNumber.clear();
							cardNumber.sendKeys(cardNum);
						}
					} else {
						if (card.toLowerCase().contains("bookmyforex")) {
							kitInput = getDriver().findElement(ElementReload.SELECT_CARD_KIT.get());
							kitInput.clear();
							kitInput.sendKeys(kit);
						} else {
							cardNumber = getDriver().findElement(ElementReload.CARD_INPUT.get());
							cardNumber.clear();
							cardNumber.sendKeys(cardNum);
						}
					}
	...
	public void duplicateCheck() throws Throwable {
		...
		for (int i = 0; i <= 3; i++) {
			...
			if (listDuplicateDialog.size() > 0 && listDuplicateDialog.get(0).isDisplayed()) {
				...
				System.err.println(
						"Tried 3 times-Seems it's a unique order/not a duplicate order/Duplicate order modal Not Shown");
